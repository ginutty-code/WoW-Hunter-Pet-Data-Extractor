...
NPC_CONCURRENCY = 50  # Increased for maximum speed with requests
BATCH_SIZE = 100
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logging.warning(f"Connection timed out. Retrying ({attempt + 2}/{max_retries})...")
                time.sleep(2)
            else:
                raise
        except requests.RequestException as e:
            if attempt < max_retries - 1:
                logging.warning(f"Connection failed: {e}. Retrying ({attempt + 2}/{max_retries})...")
                time.sleep(2)
            else:
                raise
...
"""
    ...
    # Write to file
    lua_path = os.path.join(OUTPUT_DIR, 'pet_data.lua')
    ensure_parent_dir(lua_path)
    with open(lua_path, 'w', encoding='utf-8') as f:
        f.write(lua_content)

    logging.info("Hierarchical Lua data exported to pet_data.lua")
